from muzero import Game, Node, MuZeroConfig, SharedStorage, ReplayBuffer,\
                   Network, MinMaxStats
import numpy as np
import random
from muzero.model import one_hot, extract_output
...
def expand_node(node, to_play, actions, network_output):
  ...
  _, node.reward, policy_logits, node.hidden_state = extract_output(
    network_output)
...
def run_mcts(config, root, action_history, network):
  ...
  for _ in range(config.num_simulations):
    ...
    backpropagate(search_path, value, -parent.to_play, config.discount, 
      min_max_stats)
...
def select_child(config, node, min_max_stats):
  ...
  """
  ext = [(ucb_score(config, node, child, min_max_stats), action, child)\
...
def ucb_score(config, parent, child, min_max_stats):
  ...
  on the prior.
  """
  pb_c = np.log((parent.visit_count + config.pb_c_base+1) / config.pb_c_base)\
        + config.pb_c_init
