...
class Network:
    def __init__(self, config):
        ...
        self.g = Model([hidden_state, action], [reward, hidden_state_next], name="g")
    ...
    def create_model(self):
        def softmax_ce_logits(y_true, y_pred):
            return tf.compat.v1.nn.softmax_cross_entropy_with_logits_v2(y_true, y_pred)

        observation = Input(self.observation_dim, name="o_0")
        value, reward, policy_logits, hidden_state = self.initial_inference(observation)
